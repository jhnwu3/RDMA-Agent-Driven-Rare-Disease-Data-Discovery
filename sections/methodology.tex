...
\rev{\textbf{Preliminary Filtering.} Before running RDMA refinement, we apply a set of rule-based corrections via $\Call{KeywordFilter}{}$, informed by a clinician review of the prior annotations (see Supplementary Material). This removed over 700 misannotated entries --- for example, ``MR'' had been tagged as ``Multicentric reticulohistiocytosis'' when it almost universally refers to magnetic resonance in context --- reducing the dataset from 1,073 annotations across 312 notes to 333 annotations across 117 notes (see Supplementary Material). The full set of LLM prompts used by RDMA is provided in the Supplementary Material.}
