...
\rev{Abbreviation expansion. For rare disease candidates, we first attempt to expand any clinical abbreviations via $\Call{ExpandAbbreviation}{}$ before proceeding, since abbreviation ambiguity is particularly problematic in this setting.}

\rev{Direct ontology verification. We call $\Call{OntologyMatch}{}$ to check whether the entity genuinely corresponds to an HPO or Orphanet entry in context. For rare diseases, we additionally call $\Call{LLMIsDisease}{}$ to confirm the matched entry is a disease rather than a related entity type (e.g., a gene, protein, or enzyme) that also appears in Orphanet.}

\rev{Lab event detection and implication (phenotypes only). If a phenotype entity is not directly verified, we check whether it describes an abnormal lab measurement via $\Call{IsAbnormalLabValue}{}$. If so, $\Call{LLMGenerateDirection}{}$ converts it into the corresponding implied phenotype (e.g., elevated creatinine $\to$ hypercreatininemia).}

\rev{Implied phenotype generation (phenotypes only). For entities that are neither direct phenotypes nor lab events, $\Call{LLMImpliesPhenotype}{}$ checks whether the entity contextually implies a phenotype. If so, we generate and verify the implied phenotype against the HPO ontology before accepting it.}
